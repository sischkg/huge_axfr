...
private:
    ...
    void sendResponse( const dns::PacketInfo &query, tcpv4::ConnectionPtr &conn, unsigned long long &index ) const
    {
        ...
        for ( int i = 0; i < 6; i++ ) {
            ...
            os << std::setfill( '0' ) << std::setw( 16 ) << index << "." << query_question.mDomainname;
            index++;
            ...
            answer.mDomainname = owner;
            answer.mType       = dns::TYPE_NSEC;
            answer.mClass      = dns::CLASS_IN;
            answer.mTTL        = TTL;
            answer.mRData      = dns::RDATAPtr( new dns::RecordNSEC( next_name, bitmap ) );
...
public:
    NSECAXFRServer( const std::string addr, uint16_t port, bool debug, unsigned long long count )
...
int main( int argc, char **argv )
{
    ...
    NSECAXFRServer server( bind_address, bind_port, debug, rr_count );
    server.start();
